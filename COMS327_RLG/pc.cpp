...
void delete_pc_items(character *the_pc) {
  int i;
  for (i = 0; i < 12; i++) {
    if (((pc*)(the_pc))->equip_slot[i] != NULL) {
    delete ((pc*)(the_pc))->equip_slot[i];
    }
  }
  for (i = 0; i < 10; i++) {
    if (i < 10 && ((pc*)(the_pc))->carry_slot[i] != NULL) {
      delete ((pc*)(the_pc))->carry_slot[i];
      }
  }
}
...
void config_pc(dungeon_t *d)
{
  ...
  for (i = 0; i < 12; i++) {
    if (i < 10) {
      ((pc *)(d->pc))->carry_slot[i] = NULL;
    }
    ((pc *)(d->pc))->equip_slot[i] = NULL;
  }
